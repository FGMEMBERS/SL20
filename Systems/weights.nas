...
var setWeightOptsZ = func
{
    var tankchange = 0;
    var addedTankCap = 0;
    foreach(var w; props.globals.getNode("payload").getChildren("weight"))
    {
        ...
        if(selected != nil)
        {
            foreach(var opt; w.getChildren("opt"))
            {
                ...
                {
                    w.getNode("weight-lb", 1).setValue(opt.getNode("lbs", 1).getValue());
                    break;
                }
            }
            var startInd = 0;
            ...
            if (selected.getValue() == "Bombs 100 kg")
            {
               for (var i = startInd; i < endInd; i = i + 1)
               {
    ...
    var addPerT = (addedTankCap / 10.0) + 500.0;
#    print("Added Tank " ~ addPerT);
    var tn0 = props.globals.getNode("consumables/fuel/tank[0]", 1);
    var ppg = tn0.getNode("density-ppg", 1).getValue();
    var lbs = addPerT * 2.20462262185;
    var gals = lbs / ppg;
    for (var t = 0; t < 10; t = t + 1)
    {
       var tn = props.globals.getNode("consumables/fuel/tank[" ~ t ~ "]", 1);
...
_setlistener("/sim/signals/fdm-initialized", func { settimer(setWeightOptsZ, 0) });
_setlistener("/sim/signals/reinit", func(n) { props._getValue(n, []) or setWeightOptsZ() });


# Called from the F&W dialog when the user selects a weight option
var weightChangeHandlerZepp = func {
    var tankchanged = setWeightOptsZ();
    ...
    if(tankchanged)
    {
        var p = props.Node.new({"dialog-name": "WeightAndFuelZ"});
...
# the aircraft.
#
var showWeightDialogZep = func
{
    var name = "WeightAndFuelZ";
#   menu entry is "Fuel and Payload"
    ...
    var header = gui.dialog[name].addChild("group");
    ...
    var contentArea = gui.dialog[name].addChild("group");
    contentArea.set("layout", "hbox");
    contentArea.set("default-padding", 10);

    gui.dialog[name].addChild("empty");

    var limits = gui.dialog[name].addChild("group");
    ...
    var buttonBar = gui.dialog[name].addChild("group");
	...
	if (getprop("/gear/gear[0]/wow"))
	{
		for(var i=0; i<size(wgts); i+=1)
		{
			...
			if(w.getNode("opt") != nil)
			{
				...
				combo.setBinding("nasal", "SL20.weightChangeHandlerZepp()");
			}
			else
			{
